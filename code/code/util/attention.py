...
class MemoryCompressedAttention(nn.Module):
    def __init__(
        ...
        self.register_buffer('null_k', torch.zeros(1, 1, dim))
        self.register_buffer('null_v', torch.zeros(1, 1, dim))
    ...
    def forward(self, x, input_mask = None):
        ...
        # ----------------------------------------------------------------------------------------------------------------------------------------------
        # print(self.null_k.shape, self.null_v.shape, k.shape, v.shape)
        # attach a null key and value, in the case that the first query has no keys to pay attention to
        k = torch.cat((self.null_k.broadcast_to(b, 1, d), k), dim=1)
        v = torch.cat((self.null_v.broadcast_to(b, 1, d), v), dim=1)
        # ----------------------------------------------------------------------------------------------------------------------------------------------
